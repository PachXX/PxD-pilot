...
@Controller(
  'rest/pashx-mab/procurement-cases/:procurementCaseRecordId/delivery',
)
@UseGuards(JwtAuthGuard, WorkspaceAuthGuard, CustomPermissionGuard)
export class PashxDeliveryRecordController {
  private readonly logger = new Logger(PashxDeliveryRecordController.name);

  constructor(
    private readonly capabilityService: PashxCapabilityService,
    private readonly deliveryRecordService: PashxDeliveryRecordService,
  ) {}
  ...
  async record(
    ...
    const validation = validateRecordDeliveryRequest(body);
    ...
    try {
      return await this.deliveryRecordService.record({
        workspaceId: authContext.workspace.id,
